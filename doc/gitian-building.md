...
*Setup instructions for a Gitian build of Halal Core using a Debian VM or physical system.*
...
- [Building Halal Core](#building-bitcoin-core)
...
in the Halal repository under 'Fetch and create inputs' to install sources which require
...
To build Halal Core (for Linux, OS X and Windows) just follow the steps under 'perform
...
For example:
```bash
URL=https://github.com/snguyen-vn/halal.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit halal=${COMMIT} --url bitcoin=${URL} ../bitcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit halal=${COMMIT} --url bitcoin=${URL} ../bitcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit halal=${COMMIT} --url bitcoin=${URL} ../bitcoin/contrib/gitian-descriptors/gitian-osx.yml
```
