...
const char * const BITCOIN_CONF_FILENAME = "halal.conf";
const char * const BITCOIN_PID_FILENAME = "halald.pid";
...
bool fNoDebug = false; //A temporary fix for https://github.com/halalorg/halal/issues/1011
...
{
    //A temporary fix for https://github.com/halalorg/halal/issues/1011
...
boost::filesystem::path GetDefaultDataDirForCoinName(const std::string &coinName)
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\halal
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\halal
    // Mac: ~/Library/Application Support/halal
    // Unix: ~/.halal
#ifdef WIN32
    // Windows
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    fs::path halalDefaultDir = GetDefaultDataDirForCoinName("halal");
    if (!fs::is_directory(halalDefaultDir)) {
        // try "zcoin" in case we're upgrading from pre-halal version
        fs::path zcoinDefaultDir = GetDefaultDataDirForCoinName("zcoin");
        if (fs::is_directory(zcoinDefaultDir))
            return zcoinDefaultDir;
    }

    return halalDefaultDir;
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good())
        return; // No halal.conf file is OK
...
bool RenameDirectoriesFromZcoinToHalal()
{
    namespace fs = boost::filesystem;

    fs::path zcoinPath = GetDefaultDataDirForCoinName("zcoin");
    fs::path halalPath = GetDefaultDataDirForCoinName("halal");

    // rename is possible only if zcoin directory exists and halal doesn't
    if (fs::exists(halalPath) || !fs::is_directory(zcoinPath))
        return false;

    fs::path zcoinConfFileName = zcoinPath / "zcoin.conf";
    fs::path halalConfFileName = zcoinPath / "halal.conf";
    if (fs::exists(halalConfFileName))
        return false;

    try {
        if (fs::is_regular_file(zcoinConfFileName))
            fs::rename(zcoinConfFileName, halalConfFileName);

        try {
            fs::rename(zcoinPath, halalPath);
        }
        catch (const fs::filesystem_error &) {
            // rename config file back
            fs::rename(halalConfFileName, zcoinConfFileName);
            throw;
        }
    }
    catch (const fs::filesystem_error &) {
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Halal Core copyright is not removed by accident
    if (copyright_devs.find(_("Halal Core")) == std::string::npos) {
