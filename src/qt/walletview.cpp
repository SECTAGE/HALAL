...
{
    // Create Halal transactions list
    halalTransactionList = new TransactionView(platformStyle);

    connect(halalTransactionList, SIGNAL(doubleClicked(QModelIndex)), halalTransactionList, SLOT(showDetails()));
    connect(halalTransactionList, SIGNAL(message(QString, QString, unsigned int)), this, SIGNAL(message(QString, QString, unsigned int)));

    // Create export panel for Halal transactions
    ...
    connect(exportButton, SIGNAL(clicked()), halalTransactionList, SLOT(exportClicked()));
    ...
    auto halalLayout = new QVBoxLayout();
    halalLayout->addWidget(halalTransactionList);
    halalLayout->addLayout(exportLayout);
    // TODO: fix this
    //connect(overviewPage, SIGNAL(transactionClicked(QModelIndex)), transactionView, SLOT(focusTransaction(QModelIndex)));
    connect(overviewPage, SIGNAL(outOfSyncWarningClicked()), this, SLOT(requestedSyncWarningInfo()));

    halalTransactionsView = new QWidget();
    halalTransactionsView->setLayout(halalLayout);
    ...
    if (isElysiumEnabled()) {
        ...
        transactionTabs->addTab(halalTransactionsView, tr("Halal"));
...
#endif
        pageLayout->addWidget(halalTransactionsView);
...
{
    sendHalalView = new SendCoinsDialog(platformStyle);

    connect(sendHalalView, SIGNAL(message(QString, QString, unsigned int)), this, SIGNAL(message(QString, QString, unsigned int)));
    ...
    if (isElysiumEnabled()) {
        ...
        sendCoinsTabs->addTab(sendHalalView, tr("Halal"));
...
#endif
        pageLayout->addWidget(sendHalalView);
...
void WalletView::setClientModel(ClientModel *_clientModel)
{
    ...
    sendHalalView->setClientModel(clientModel);
...
void WalletView::setWalletModel(WalletModel *_walletModel)
{
    ...
    halalTransactionList->setModel(_walletModel);
    ...
    sendHalalView->setModel(_walletModel);
...
void WalletView::focusBitcoinHistoryTab(const QModelIndex &idx)
{
    ...
    halalTransactionList->focusTransaction(idx);
}
...
bool WalletView::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    ...
    return sendHalalView->handlePaymentRequest(recipient);
}
