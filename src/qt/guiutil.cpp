...
static QString halalTheme = "halalTheme";
static QString halalLowTheme = "halalLowTheme";
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Halal address (e.g. %1)").arg(
...
{
    // return if URI is not valid or is no halal: URI
    if(!uri.isValid() || uri.scheme() != QString("halal"))
        return false;
...
{
    // Convert halal:// to halal:
    //
    //    Cannot handle this later, because halal:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("halal://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "halal:");
    }
...
{
    QString ret = QString("halal:%1").arg(info.address);
    int paramCount = 0;
...
boost::filesystem::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Halal.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Halal (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Halal (%s).lnk", chain);
}
...
boost::filesystem::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "halal.desktop";
    return GetAutostartDir() / strprintf("halal-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a halal.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Halal\n";
        else
            optionFile << strprintf("Name=Halal (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Halal app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add Halal app to startup item list
...
void loadTheme()
{
    ...
    QString fileName = stylesheetDirectory + "/" + halalTheme;
    ...
    if (QApplication::desktop()->screenGeometry().height() <= 800) {
        fileName = stylesheetDirectory + "/" + halalLowTheme;
