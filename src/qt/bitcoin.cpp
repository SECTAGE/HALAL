...
{
    return QCoreApplication::translate("halal-core", psz).toStdString();
}
...
public:
    ...
    // migrate settings to halal. Returns true if there was migration
    ...
    // migrate directories to halal if needed
    void migrateToHalal();
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if(retval)
    {
        ...
        // halal: URIs or payment requests:
...
{
    QMessageBox::critical(0, "Runaway exception", BitcoinGUI::tr("A fatal error occurred. Halal can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
void BitcoinApplication::migrateToHalal()
{
    migrateSettings("Zcoin", "Halal", "Zcoin-Qt", "Halal-Qt");
    migrateSettings("Zcoin", "Halal", "Zcoin-Qt-testnet", "Halal-Qt-testnet");
    ...
    boost::filesystem::path halalDefaultDataDir = GetDefaultDataDirForCoinName("halal");

    if (dataDir != zcoinDefaultDataDir)
        return;

    boost::filesystem::path dontMigrateFilePath = dataDir / ".dontmigratetohalal";
    if (boost::filesystem::exists(dontMigrateFilePath) && !GetBoolArg("-migratetohalal", false))
        return;
    ...
    QTextStream(&messageText) <<
        "Migrate directory structure from zcoin to halal? "
        "Directory " << GUIUtil::boostPathToQString(zcoinDefaultDataDir) <<
          " will be renamed to " << GUIUtil::boostPathToQString(halalDefaultDataDir) <<
          " and file zcoin.conf in it will be renamed to halal.conf";
    ...
    if (messageBox.exec() == QMessageBox::Yes) {
        if (RenameDirectoriesFromZcoinToHalal()) {
            // Update path in settings if not set to non-default value
            setDataDirInSettings("Halal", "Halal-Qt", GUIUtil::boostPathToQString(halalDefaultDataDir));
            setDataDirInSettings("Halal", "Halal-Qt-testnet", GUIUtil::boostPathToQString(halalDefaultDataDir));
        }
    }
...
int main(int argc, char *argv[])
{
    ...
    /// 6. Determine availability of data directory and parse halal.conf
    ...
    // halal: links repeatedly have their payment requests routed to this process:
