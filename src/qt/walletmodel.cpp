...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
    ...
    Q_FOREACH(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        {   // User-entered Halal address / amount:
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction &transaction)
{
...

    {
        ...
        Q_FOREACH(const SendCoinsRecipient &rcp, transaction.getRecipients())
        {
            ...
            else if (!rcp.message.isEmpty()) // Message from normal halal:URI (halal:123...?message=example)
...

    {
        ...
        for (const auto& rcp : transaction.getRecipients()) {
            if (rcp.paymentRequest.IsInitialized())
            {
                ...
                // Message from normal halal:URI (halal:123...?message=example)
...
void WalletModel::handleBip47Keys(int receiverAccountNum, void * pBlockIndex_)
{
    ...
    if (wallet->GetBip47Wallet()) {
        ...
        while(!ctx.isValid()) {
            ...
            msgDialog.setInformativeText(tr("If you do not enter your password now, you will need to rescan your wallet to receive your HALAL.<br/><br/>Re-enter your password?"));
