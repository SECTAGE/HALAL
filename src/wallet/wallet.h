...
const uint32_t BIP44_HALAL_INDEX = 0x88; // https://github.com/satoshilabs/slips/blob/master/slip-0044.md#registered-coin-types
...
public:
    ...
    // znode
    /// Get 1000 HALAL output and keys which can be used for the Znode
    ...
    /** \brief Selects coins to spend, and coins to re-mint based on the required amount to spend, provided by the user. As the lower denomination now is 0.1 halal, user's request will be rounded up to the nearest 0.1. This difference between the user's requested value, and the actually spent value will be left to the miners as a fee.
     ...
     * \returns true, if it was possible to spend exactly required(rounded up to 0.1 halal) amount using coins we have.
     */
