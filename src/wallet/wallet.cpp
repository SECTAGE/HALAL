...
CPubKey CWallet::GetKeyFromKeypath(uint32_t nChange, uint32_t nChild, CKey& secret) {
    ...
    uint32_t nIndex = Params().GetConsensus().IsMain() ? BIP44_HALAL_INDEX : BIP44_TEST_INDEX;
    ...
    CExtKey coinTypeKey;           //key at m/44'/<1/136>' (Testnet or Halal Coin Type respectively, according to SLIP-0044)
...
CBlockIndex* CWallet::ScanForWalletTransactions(CBlockIndex *pindexStart, bool fUpdate, bool fRecoverMnemonic)
{
    ...
    CBlockIndex* pindex = pindexStart;
    {
        ...
        // if you are recovering wallet with mnemonics start rescan from block when mnemonics implemented in Halal
        if (fRecoverMnemonic) {
        ...
        const CCoinControl *coinControl) const
{
    ...
    if (!MoneyRange(required)) {
        throw std::invalid_argument("Request to spend more than 21 MLN halals.\n");
    }
