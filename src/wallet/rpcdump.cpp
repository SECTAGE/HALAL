...
UniValue importprivkey(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 3)
        throw std::runtime_error(
            "importprivkey \"halalprivkey\" ( \"label\" ) ( rescan )\n"
            "\nAdds a private key (as returned by dumpprivkey) to your wallet.\n"
            "\nArguments:\n"
            "1. \"halalprivkey\"   (string, required) The private key (see dumpprivkey)\n"
    ...
    if(chain.nVersion == chain.VERSION_WITH_BIP39){
        throw JSONRPCError(RPC_WALLET_ERROR, "Importing wallets and private keys is disabled for mnemonic-enabled wallets."
                                             "To import your dump file, create a non-mnemonic wallet by setting \"usemnemonic=0\" in your halal.conf file, after backing up and removing your existing wallet.");
    }
...
UniValue importaddress(const JSONRPCRequest& request)
{
    ...
    if (address.IsValid()) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Halal address or script");
    }
...
UniValue dumpprivkey(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"   (string, required) The Halal address for the private key\n"
            "\nResult:\n"
...
UniValue dumpprivkey_halal(const JSONRPCRequest& request)
{
#ifndef UNSAFE_DUMPPRIVKEY
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            "dumpprivkey \"halaladdress\"\n"
            "\nReveals the private key corresponding to 'halaladdress'.\n"
            "Then the importprivkey can be used with this output\n"
            "\nArguments:\n"
            "1. \"halaladdress\"   (string, required) The Halal address for the private key\n"
    ...
    if (request.params.size() == 1 || !AuthorizationHelper::inst().authorize(__FUNCTION__ + request.params[0].get_str(), request.params[1].get_str()))
    {
        std::string warning =
            ...
            "Halal team members will never ask for this command's output and it is not needed for Znode setup or diagnosis!\n"
            "\n"
            " Please seek help on one of our public channels. \n"
            " Telegram: https://t.me/halalproject \n"
            " Discord: https://discord.com/invite/TGZPRbRT3Y\n"
            " Reddit: https://www.reddit.com/r/HalalProject/\n"
            "\n"
            ;
...
UniValue dumpwallet(const JSONRPCRequest& request)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by Halal %s\n", CLIENT_BUILD);
...
UniValue dumpwallet_halal(const JSONRPCRequest& request)
{
#ifndef UNSAFE_DUMPPRIVKEY
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
