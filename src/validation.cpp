...
#if defined(NDEBUG)
# error "Halal cannot be compiled without assertions."
#endif
...
bool AcceptToMemoryPoolWorker(CTxMemPool& pool, CValidationState& state, const CTransactionRef& ptx, bool fLimitFree,
                              ...
                              bool isCheckWalletTransaction, bool markHalalSpendTransactionSerial)
{
    ...
    if (tx.IsSigmaSpend()) {
        if(markHalalSpendTransactionSerial)
                        ...
                        bool isCheckWalletTransaction, bool markHalalSpendTransactionSerial)
{
    ...
    bool res = AcceptToMemoryPoolWorker(pool, state, tx, fLimitFree, pfMissingInputs, nAcceptTime, plTxnReplaced, fOverrideMempoolLimit, nAbsurdFee, coins_to_uncache, isCheckWalletTransaction, markHalalSpendTransactionSerial);
    if (!res) {
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState &state, const CTransactionRef &tx, bool fLimitFree,
                        ...
                        bool isCheckWalletTransaction, bool markHalalSpendTransactionSerial)
{
    return AcceptToMemoryPoolWithTime(pool, state, tx, fLimitFree, pfMissingInputs, GetTime(), plTxnReplaced, fOverrideMempoolLimit, nAbsurdFee, isCheckWalletTransaction, markHalalSpendTransactionSerial);
}


bool AcceptToMemoryPool(CTxPoolAggregate& poolAggregate, CValidationState &state, const CTransactionRef &tx, bool fLimitFree,
                        bool* pfMissingInputs, std::list<CTransactionRef>* plTxnReplaced,
                        bool fOverrideMempoolLimit, const CAmount nAbsurdFee, bool isCheckWalletTransaction, bool markHalalSpendTransactionSerial) {
    bool res = AcceptToMemoryPool(mempool, state, tx, fLimitFree, pfMissingInputs, plTxnReplaced, fOverrideMempoolLimit, nAbsurdFee, isCheckWalletTransaction, markHalalSpendTransactionSerial);
        ...
        BOOST_FOREACH(const CTxIn &txin, tx.vin) {
            if(!txin.scriptSig.IsSigmaSpend()) {
                return state.DoS(
                    100, false,
                    REJECT_MALFORMED,
                    "CheckSpendHalalTransaction: can't mix zerocoin spend input with regular ones");
            }
        ...
        for (const auto& it : block.vtx) {
            ...
            if (!tx.IsCoinBase()) {
                AcceptToMemoryPool(
                    ...
                    false /* markHalalSpendTransactionSerial */
                );
            }
...
//    if (nHeight > nMNPIBlock + (nMNPIPeriod * 9)) ret += blockValue / 40; // 313520 - 50.0% - 2015-08-03
    ...
    CAmount ret = 15 * coin; //15 or 7.5 HALAL
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    ...
    if (nHeight > 1) {
                ...
                CAmount devPayoutValue = (GetBlockSubsidy(nHeight, consensusParams) * consensusParams.stage2DevelopmentFundShare) / 100;
                bool found = false;
...
static CBlock CreateGenesisBlock(const char* pszTimestamp, const CScript& genesisOutputScript, uint32_t nTime, uint32_t nNonce, uint32_t nBits, int32_t nVersion, const CAmount& genesisReward, std::vector<unsigned char> extraNonce)
{
    ...
    txNew.vin.resize(1);
    txNew.vout.resize(1);
    txNew.vin[0].scriptSig = CScript() << 504365040 << CBigNum(4).getvch() << std::vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp)) << extraNonce;
...
void findgenesis()
{
    ...
    extraNonce[2] = 0x00;
    extraNonce[3] = 0x00;
    uint256 powLimit = uint256S("0x00000fffffffffffffffffffffffffffffffffffffffffffffffffffffffffff");
    while (UintToArith256(genesis.GetPoWHash(0)) > UintToArith256(powLimit)) {
        nNonce++;
        genesis = CreateGenesisBlock(1657684800, nNonce, 0x1e0ffff0, 2, 0, extraNonce);
    }
    genesis = CreateGenesisBlock(1657684800, nNonce, 0x1e0ffff0, 2, 0, extraNonce);
    uint256 hashGenesisBlock = genesis.GetPoWHash(0);
    ...
    if (!fReindex) {
        try {
            CBlock &block = const_cast<CBlock&>(chainparams.GenesisBlock());
             ...
             LogPrintf(">>>>>>>>>>GENESIS>>>>>>>>>>>>>>%s", block.ToString().c_str());
