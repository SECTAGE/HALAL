    ...
    for (std::size_t j = 0; j < sigmaProofs.size(); j += threadsMaxCount) {
        ...
        if (isFail) {
            ...
            throw std::invalid_argument(
                    "Sigma batch verification failed, please run Halal with -reindex -batching=0");
        }
        parallelTasks.clear();
    }
    if (!sigmaProofs.empty())
...
void BatchProofContainer::batch_lelantus() {
    ...
    for (std::size_t j = 0; j < lelantusSigmaProofs.size(); j += threadsMaxCount) {
        ...
        if (isFail) {
            ...
            throw std::invalid_argument("Lelantus batch verification failed, please run Halal with -reindex -batching=0");
        }
...
void BatchProofContainer::batch_rangeProofs() {
    ...
    for (const auto& itr : rangeProofs) {
        ...
        if (!rangeVerifier.verify(V, commitments, proofs)) {
            ...
            throw std::invalid_argument("RangeProof batch verification failed, please run Halal with -reindex -batching=0");
        }
    }
