...
def activate_sigma_spend(denoms, spendsize, exp_spends, exp_remints, halald):
    for denom in denoms:
        count, size = denom
        for i in range(count):
            halald.nodes[0].mint(size)
    halald.nodes[0].generate(6)

    myaddr = halald.nodes[0].listreceivedbyaddress(0, True)[0]['address']
    args = {myaddr: spendsize}
    txid = halald.nodes[0].spendmany("", args)
    halald.nodes[0].generate(2)

    # Should be checked spends
    spends = halald.nodes[0].listsigmaspends(0)
