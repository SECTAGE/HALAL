...
def initialize_datadir(dirname, n):
    ...
    with open(os.path.join(datadir, "halal.conf"), 'w', encoding='utf8') as f:
        f.write("regtest=1\n")
...
def wait_for_bitcoind_start(process, url, i):
    '''
    Wait for halald to start. This means that RPC is accessible and fully initialized.
    Raise an exception if halald exits during initialization.
    '''
    while True:
        if process.poll() is not None:
            raise Exception('halald exited with status %i during initialization' % process.returncode)
        try:
...
def initialize_chain(test_dir, num_nodes, cachedir):
    ...
    if create_cache:
        ...
        for i in range(MAX_NODES):
            ...
            args = [ os.getenv("HALALD", "halald"), "-server", "-keypool=1", "-datadir="+datadir, "-discover=0" ]
            if i > 0:
...
def start_node(i, dirname, extra_args=None, rpchost=None, timewait=None, binary=None, redirect_stderr=False, stderr=None):
    ...
    if binary is None:
        binary = os.getenv("HALALD", "halald")
...
# Don't try auto backups (they fail a lot when running tests)
    ...
    logger.debug("start_node: halald started, waiting for RPC to come up")
